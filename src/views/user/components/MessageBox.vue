<template>
  <div class="message-box">
    <mu-appbar style="width: 100%;" color="primary" title="Liteam" ref="indexAppBar">
...
export default {
  ...
  methods: {
    ...
    toUserDetail() {
      this.$router.push(`/user/detail?liteam=${this.user.curaNumber}`);
    },
    handleLogout() {
      this.$router.replace(
        `/login?liteam=${this.user.curaNumber}&headUrl=${this.user.headUrl}`
      );
      this.logout();
