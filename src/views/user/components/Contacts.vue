      ...
      <div :class="searchWrapperCls">
        <mu-flex direction="column" class="search-contact-wrapper" align-items="center">
          ...
          <mu-list class="search-result-wrapper" v-if="searchRes.length !== 0">
            ...
            <mu-list-item v-for="contact in searchRes" :key="contact.curaNumber" avatar button :to="`/user/chat?liteam=${contact.curaNumber}`">
              <mu-list-item-action>
                <mu-avatar>
  ...
  methods: {
    ...
    handleChatClick(curaNumber) {
      if (curaNumber == this.user.curaNumber) {
        this.$router.push("/user/detail?liteam=" + curaNumber)
      } else {
        this.$router.push("/user/chat?liteam=" + curaNumber);
      }
    },
    searchSubmit() {
